...
public class PetriNetTransformer {
     ...
     * 
     * First version with transforming fairness assumptions given at the original transitions
     * to subnets with fairness annotations to the transitions.
     * 
     * @param game
     * @return 
     */
    public static PetriGame createNet4ModelChecking(PetriGame game) {
        ...
        while (!todo.isEmpty()) {
            ...
            for (Transition t : pOrig.getPostset()) { // for all transitions of the place add for all token flows a new transition
                ...
                Place buffer = null;
                // %%%%%%%%%%%%%%%%% FAIR
                if (tfl.getPostset().size() > 1 && game.isFair(t)) { // if the transition is fair and we have more then one token flow successor,
                ...
                for (Place post : tfl.getPostset()) {
                    ...
                    Transition tout = out.createTransition(); // create the new transition
                    tout.setLabel(t.getId());
                    if (game.isFair(t) && tfl.getPostset().size() <= 1) { // a fair transition which has only one token flow successor, thus is not handeled above, set it here to be fair
                        out.setFair(tout);
                    ...
                    if (!game.isFair(t) || tfl.getPostset().size() <= 1) {
                    ...
                    if (tfl.getPostset().size() > 1 && game.isFair(t)) { // reactivate all but those transition which can succeed the flow in the next step
        ...
        for (Transition t : game.getTransitions()) { // delete the fairness assumption of all original transitions
            out.removeFair(out.getTransition(t.getId()));
        }
        return out;
    }
}
